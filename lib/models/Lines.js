...
class LineQueryBuilder extends CommonQueryBuilder {
   ...
   * @param strict Search using first letter start. Anywhere by default.
   */
  genericSearch( search, field, orderResults = true, strict = false ) {
    let query = this
    ...
    // Search first letters only if true, otherwise anywhere
    const value = strict ? `${asciiSearch}%` : `%${asciiSearch}%`
   ...
   * @param strict Search using first letter start. Anywhere by default.
   */
  firstLetters( letters, orderResults = true, strict = false ) {
    return this.genericSearch( letters, 'first_letters', orderResults, strict )
  }
