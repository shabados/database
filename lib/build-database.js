...
const validateLine = callback => data => {
  const { id, gurmukhi } = data

  if ( !gurmukhi ) throw new Error(`Line ID ${id} has no gurmukhi`)
  ...
  const sourceData = await Promise.all( (
    sourceNames.map( async ( sourceName, sourceIndex ) => {
      ...
      const data = map( shabads, ( {
        ...
        map( lines, validateLine( ( { type, translations, gurmukhi, id: lineId, ...rest } ) => ( [
