/**
 * Generates a DB from the JSON sources in `data/`.
 */
...
const publicationNames = toEnglishName( require( '../data/publications' ) )
    ...
    sourceNames.map( async ( sourceName, sourceIndex ) => {
      ...
      const data = map( shabads, ( {
        ...
        map( lines, ( { type, translations, gurmukhi, id: lineId, ...rest } ) => ( [
          // First element is lines
          {
            ...
            gurmukhi,
            first_letters: firstLetters( gurmukhi[ publicationNames[ 0 ] ] ), // SGPC as gurmukhi input
            vishraam_first_letters: firstLetters( gurmukhi[ publicationNames[ 0 ] ], true, true ),
            shabad_id: shabadId,
      ...
      forEach( data, ( [ , lines ] ) => (
        forEach( lines, ( [ line, translations ] ) => {
          ...
          // Use SGPC gurmukhi as default for transliterations etc
          const sgpcGurmukhi = gurmukhi[ publicationNames[ 0 ] ]

          transliterateAll( sgpcGurmukhi ).forEach( ( [ languageId, transliteration ] ) =>
          ...
          lineData.push( { ...line, gurmukhi: sgpcGurmukhi } )

          // Add each publication's gurmukhi
          Object.entries( gurmukhi ).forEach( ( [ publicationName, gurmukhi ] ) => {
            lineContentData.push( {
              line_id: line.id,
              publication_id: (
                publicationNames.findIndex( nameEnglish => publicationName === nameEnglish )
                + 1
              ),
              gurmukhi,
            } )
