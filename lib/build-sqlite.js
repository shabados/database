...
/**
 * Imports translations for a given source and set of shabads.
 * @param {Object} trx The transaction to use when executing any queries.
 * @param {Object} sources The shabads with lines and translations per source.
 */
const importTranslations = async ( trx, sources ) => {
  console.log( '\nImporting table translations'.subheader )

  await Promise.all( map( Object.entries( sources ), async ( [ sourceName, shabads ] ) => {
    console.log( `Processing translations for ${sourceName}` )

    // Flatten shabads
    const data = reduce( shabads, ( acc, { lines } ) => ( [
      ...acc,
      // Flatten lines
      ...reduce( lines, ( acc, { id: lineId, translations } ) => ( [
        ...acc,
        ...
        ...reduce( Object.entries( translations ), ( acc, [ languageName, translations ] ) => [
          ...
          ...reduce( Object.entries( translations ), ( acc, [
            ...
            { translation, additional_information },
          ] ) => [
            ...acc,
            {
              line_id: lineId,
    ...
    await Promise.all( map( data, translation => trx( 'translations' ).insert( translation ) ) )
  } ) )

  console.log( 'Successfully imported table translations'.success )
}
...
const importLines = async trx => {
  console.log( '\nImporting tables lines, shabads'.subheader )
  ...
  const sourceData = await Promise.all( (
    sourceNames.map( async ( sourceName, sourceIndex ) => {
      ...
      const shabads = reduce( readdirSync( sourceDir ), ( acc, path ) => [
        ...acc,
      ...
      const lineData = map( shabads, ( { writer, section, subsection, lines, ...rest } ) => ( {
        ...rest,
        orderId: shabadId++, // eslint-disable-line
        sourceId,
        ...
        lines: map( lines, ( { lineType, translations, ...rest } ) => ( {
          ...rest,
      ...
      // Insert each shabad one by one, with lines attached
      await Promise.all( map( lineData, shabad => Shabads.query( trx ).insertGraph( shabad ) ) )
...
const main = async () => {
  ...
  await transaction( knex, async trx => {
    ...
    const sourceData = await importLines( trx )
    await importTranslations( trx, sourceData )
