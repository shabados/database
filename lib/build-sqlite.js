...
const importSimpleTables = async trx => (
  Promise.all( [
    ...
    .map( async ( [ name, model ] ) => {
      console.log( `\nImporting table ${name}`.subheader )
          ...
          .map( ( { writer, section, subsection, lines, ...rest }, index ) => ( {
            ...
            lines: lines.map( ( { translations, lineType, ...rest } ) => ( {
              ...
              translations: Object
                ...
                .reduce( ( acc, [ languageName, { additionalInformation, ...translations } ] ) => [
                  ...acc,
                  ...Object
                    .entries( translations )
                    .reduce( ( acc, [ translationSourceName, translation ] ) => [
                      ...acc,
                      {
                        ...
                        ...translation,
                        additionalInformation: JSON.stringify( additionalInformation ), // JSON
                      },
                    ], [] ),
                ], [] ),
            } ) ),
