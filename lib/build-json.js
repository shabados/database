...
const processLines = async () => {
  ...
  return Sources
    ...
    .then( sources => sources.reduce( async ( acc, { shabads, nameEnglish } ) => ( {
      ...( await acc ),
      [ nameEnglish ]: await Promise.all( shabads.map( async shabad => ( { //! Do not destructure - need model
      // Generate JSON with desired keys for Shabad
        ...
        sttm2_id: shabad.sttm2Id, //! Snakecase converter doesn't work with number
        // Generate JSON for lines for that shabad with desired keys
        lines: ( await shabad
          .$relatedQuery( 'lines' )
          .orderBy( 'order_id' )
          .eager( '[type, translations.translationSource.language]' )
        ).map( ( {
          orderId,
          firstLetters,
          typeId,
