...
const availableTypes = [ 'shabad', 'line' ]
...
/**
  ...
  * @param {String} type check `availableTypes`
  */
const logGenerateIds = async ( count, type ) => {
  const newId = idTypes[ type ]

  // Grab ids to exclude from database
  const ids = new Set( ( await newId[ 0 ].query().select( 'id' ) ).map( ( { id } ) => id ) )

  // Loop to print id's
  for ( let i = 0; i < count; i += 1 ) {
    console.log( `${await findId( ids, newId[ 1 ] )}` )
  }
}
